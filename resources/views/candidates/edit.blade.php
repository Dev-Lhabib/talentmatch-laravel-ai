...
@section('content')
    <h1 class="mb-6 text-xl font-semibold text-white">Modifier le candidat</h1>
    ...
    <form method="POST" action="{{ route('candidates.update', $candidate) }}">
        ...
        <div class="mt-6 flex gap-2">
            ...
            <a href="{{ route('candidates.show', $candidate) }}" class="rounded-lg border border-border px-4 py-2.5 text-sm text-text-secondary transition hover:bg-card-hover hover:text-white">
                Annuler
            </a>
        </div>
    </form>
