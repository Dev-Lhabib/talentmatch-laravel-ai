    ...
    <div class="mb-6 flex items-start justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-text-secondary">
                Soumise le {{ $candidate->created_at->format("d/m/Y") }} pour l'offre « {{ $offre->titre }} »
            </p>
        </div>
        <form method="POST" action="{{ route("offres.candidatures.destroy", [$offre, $candidate]) }}" onsubmit="return confirm("Êtes-vous sûr de vouloir supprimer cette candidature ?");" class="inline">
            @csrf
            @method("DELETE")
    ...
    @if($candidate->analyse)
        <div class="mb-6 rounded-xl border border-border bg-card p-5">
            ...
            <div class="mb-4">
                ...
                @if(!empty($candidate->analyse->points_forts))
                    <ul class="space-y-0.5 pl-4 text-sm text-text-secondary">
                        @foreach($candidate->analyse->points_forts as $point)
            ...
            <div class="mb-4">
                ...
                @if(!empty($candidate->analyse->lacunes))
                    <ul class="space-y-0.5 pl-4 text-sm text-text-secondary">
                        @foreach($candidate->analyse->lacunes as $lacune)
            ...
            <div class="mb-4">
                ...
                @if(!empty($candidate->analyse->competences_manquantes))
                    <div class="flex flex-wrap gap-1.5">
                        @foreach($candidate->analyse->competences_manquantes as $competence)
            ...
            <div>
                ...
                <p class="text-sm leading-relaxed text-text-secondary">{{ $candidate->analyse->justification }}</p>
            </div>
        </div>
    @endif
